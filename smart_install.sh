...
if [ "$install_python_major_version" -eq "3"]; then
    ...
    if [ "$?" -eq "0" ] || [ ! -z "$python2_version" ]; then
        maj="$(echo $python2_version | sed -rn 's/^.+([[:digit:]])+.([[:digit:]])+.+$/\1/p')"
        min="$(echo $python2_version | sed -rn 's/^.+([[:digit:]])+.([[:digit:]])+.+$/\2/p')"
    else
        maj="0"
        min="0"
    fi
...
if [ "$maj" -eq "2" ] && [ "$min" -eq "7"]; then
    python2_prefix="$(python2 -c 'import sys; print(sys.bsae_exec_prefix)')"
...
# Perl must be v5.26+ to compile the correct version of SWIG. Note that 6+ is
# untested.
cd $base
...
# If the installed version is insufficient, temporarily instal 8.40
