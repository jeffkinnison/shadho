...
class BaseResult(object):
    ...
    def to_feature_vector(self):
        ...
        `id` attribute of the domain that generated them. This ensures that the
        order always the same for a particular model or set of search domains.
        """
        v = np.zeros(len(self.values) + 1, dtype=np.float64)
        self.values.sort(key=lambda x: x.domain_id)
